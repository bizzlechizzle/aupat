...
#project #overview #aupat #planning
...
2. [[db_import]] - Import location and media to staging area
...
6. [[db_verify]] - Verify SHA256 integrity, cleanup staging
7. [[db_identify]] - Generate master JSON export per location
8. [[database_cleanup]] - Maintenance and integrity checks

**Utility Operations**:
- [[backup]] - Create timestamped database backups
- [[gen_uuid]] - Generate unique UUID4 identifiers
- [[gen_sha]] - Generate SHA256 hashes for files
- [[name]] - Generate standardized file names
- [[folder]] - Create folder structures
...
#archive #media-organization

Media files are organized in the archive location (specified in [[user_json]]) using this structure:
...
- Creates all tables ([[locations_table]], [[sub-locations_table]], [[images_table]], [[videos_table]], [[documents_table]], [[urls_table]], [[versions_table]])
- Manages schema version tracking
- Performs schema updates/migrations
- Enforces PRAGMA foreign_keys = ON

**Dependencies**:
- [[user_json]] (database paths)
...
- [[gen_sha]] (SHA256 hashing)
- [[user_json]] (database and ingest paths)
...
#### 3. [[db_organize]]
#metadata #exif #hardware #categorization
...
2. Process approved extensions (from [[approved_ext_json]])
...
- [[camera_hardware_json]] (hardware classification)
- [[live_videos_json]] (live photo matching rules)
- [[approved_ext_json]] (special extension handling)
- [[ignored_ext_json]] (excluded extensions)
...
#### 4. [[db_folder]]
#folders #structure #organization

**Purpose**: Create organized folder structure for location

**Operations**:
1. Read folder structure from [[folder_json]]
...
**Dependencies**:
- [[folder_json]] (folder structure template)
- [[user_json]] (archive location path)
...
#### 5. [[db_ingest]]
#ingest #file-movement #hardlink #archive
...
**For Images**:
1. Generate standardized filename using [[name]]:
...
#### 6. [[db_verify]]
#verification #integrity #sha256 #cleanup
...
#### 8. [[database_cleanup]]
#maintenance #cleanup #integrity #backups
...
**Dependencies**:
- [[user_json]] (database and backup paths)
- Database
...
#### 10. [[gen_uuid]]
#uuid #identifier #collision-detection
...
#### 11. [[gen_sha]]
#sha256 #hashing #integrity #deduplication
...
#### 12. [[name]]
#naming #filenames #standardization

**Purpose**: Generate standardized file names

**Operations**:

**File Naming Patterns** (from [[name_json]]):
...
#### 13. [[folder]]
#folders #creation #templates

**Purpose**: Create folder structures based on configuration

**Operations**:
1. Read folder structure definition from [[folder_json]]
...
**Folder Categories** (from [[folder_json]]):
...
#### 2. [[sub-locations_table]]
#database #sub-locations #schema
...
- `loc_uuid` (TEXT, FOREIGN KEY → [[locations_table]].loc_uuid) - Parent location
...
**Foreign Keys**:
- loc_uuid REFERENCES [[locations_table]](loc_uuid) ON DELETE CASCADE
...
**Relationships**:
- `loc_uuid` (TEXT, FOREIGN KEY → [[locations_table]].loc_uuid) - Parent location
- `sub_uuid` (TEXT, FOREIGN KEY → [[sub-locations_table]].sub_uuid, nullable) - Parent sub-location
...
- Matched against [[camera_hardware_json]] rules
- DSLR: Canon, Nikon, Sony, Fujifilm, Olympus, Pentax, Leica, Panasonic, Hasselblad
- Drone: DJI, Autel, Parrot, Skydio, Yuneec
- Phone: iPhone, Samsung, Google, OnePlus, Huawei, Xiaomi, etc.
- GoPro: GoPro models
...
**Foreign Keys**:
- loc_uuid REFERENCES [[locations_table]](loc_uuid) ON DELETE CASCADE
- sub_uuid REFERENCES [[sub-locations_table]](sub_uuid) ON DELETE SET NULL
...
- Matched against [[camera_hardware_json]] rules
- Live videos detected by matching to images via [[live_videos_json]] rules
...
- Other extensions as specified in [[approved_ext_json]]
...
#### 9. [[approved_ext_json]]
#configuration #extensions #documents
...
#### 10. [[ignored_ext_json]]
#configuration #extensions #exclusions
...
#### 11. [[live_videos_json]]
#configuration #live-photos #matching
...
#### 12. [[folder_json]]
#configuration #folders #templates
...
**Template Variables**:
- {arch_loc} - Archive root from [[user_json]]
...
#### 13. [[name_json]]
#configuration #naming #filenames
...
**Tables**:
1. [[locations_table]] - Main locations
2. [[sub-locations_table]] - Sub-locations within locations
3. [[images_table]] - Image files
4. [[videos_table]] - Video files
5. [[documents_table]] - Document files
6. [[urls_table]] - URL references
7. [[versions_table]] - Version tracking

**Relationships**:
- [[locations_table]] → [[sub-locations_table]] (one-to-many)
- [[locations_table]] → [[images_table]] (one-to-many)
- [[locations_table]] → [[videos_table]] (one-to-many)
- [[locations_table]] → [[documents_table]] (one-to-many)
- [[locations_table]] → [[urls_table]] (one-to-many)
- [[sub-locations_table]] → [[images_table]] (one-to-many, nullable)
- [[sub-locations_table]] → [[videos_table]] (one-to-many, nullable)
- [[sub-locations_table]] → [[documents_table]] (one-to-many, nullable)
- [[sub-locations_table]] → [[urls_table]] (one-to-many, nullable)
- [[images_table]] ↔ [[videos_table]] (many-to-many via JSON1 fields, for live photos)
- [[images_table]] ↔ [[documents_table]] (many-to-many via JSON1 fields)
- [[videos_table]] ↔ [[documents_table]] (many-to-many via JSON1 fields)
...
- [[locations_table]]: loc_uuid (PRIMARY KEY), loc_uuid8 (UNIQUE)
- [[sub-locations_table]]: sub_uuid (PRIMARY KEY), sub_uuid8 (UNIQUE)
- [[images_table]]: img_sha256 (UNIQUE)
- [[videos_table]]: vid_sha256 (UNIQUE)
- [[documents_table]]: doc_sha (UNIQUE)
- [[urls_table]]: url_uuid (PRIMARY KEY)
- [[versions_table]]: modules (PRIMARY KEY)

**Composite Indexes** (for common queries):
- [[locations_table]]: (state, type) - for browsing by geography/type
- [[images_table]]: (loc_uuid, camera) - for filtering by location and hardware
- [[videos_table]]: (loc_uuid, drone) - for filtering by location and hardware
...
**JSON Fields**:
- [[images_table]].img_hardware - Structured hardware metadata: {make, model, category}
- [[videos_table]].vid_hardware - Structured hardware metadata: {make, model, category}
- [[images_table]].img_docs - Array of related document SHA256s
- [[images_table]].img_vids - Array of related video SHA256s (live photos)
- [[videos_table]].vid_docs - Array of related document SHA256s
- [[videos_table]].vid_imgs - Array of related image SHA256s (live videos)
...
- Before schema migrations ([[db_migrate]])
- Before bulk imports ([[db_import]] with many files)
- Before cleanup operations ([[database_cleanup]])
- On user request ([[backup]])
...
**Post-Import Verification** ([[db_verify]]):
...
**Database Integrity Check** ([[database_cleanup]]):
...
**2. Database Migration** ([[db_migrate]])
```
...
**4. Organize and Categorize** ([[db_organize]])
```
For each image:
- Run exiftool to extract metadata
- Parse Make/Model from EXIF
- Match against [[camera_hardware_json]]
...
- Check [[approved_ext_json]] for special handling
...
**5. Create Folder Structure** ([[db_folder]])
```
- Read folder template from [[folder_json]]
...
**8. Generate JSON Export** ([[db_identify]])
```
...
**9. Cleanup and Maintenance** ([[database_cleanup]])
```
...
- [ ] Create [[user_json]] configuration file
...
**Export**:
- [ ] Implement [[db_identify]]
- [ ] Test JSON export generation
- [ ] Validate JSON output

**Maintenance**:
- [ ] Implement [[database_cleanup]]
...
- Methodology defined: [[claudecode]] complete
- Architecture documented: [[project-overview]] complete
...
3. **Create [[user_json]]**: Configure database paths
4. **Create JSON schema files**: All 15 files in data/
5. **Implement utilities first**: [[gen_uuid]], [[gen_sha]], [[backup]], [[name]], [[folder]]
6. **Implement database**: [[db_migrate]] to create schema
...
**Related Pages**: [[claude]], [[claudecode]]
